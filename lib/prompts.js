  ...
  inquirer.prompt(viewBy).then(async (res) => {
    ...
    switch (res.viewBy) {
      ...
      case 'department':
        ...
        optionsQuery('SELECT name FROM department', 'name').then(() => {
          return inquirer
            .prompt(selectPrompt)
      ...
      case 'role':
        ...
        optionsQuery('SELECT title FROM role', 'title').then(() => {
          return inquirer
            .prompt(selectPrompt)
...
const add = async () => {
  inquirer.prompt(addSelect).then(async (res) => {
    switch (res.add_select) {
      case 'employee':
        ...
        inquirer.prompt(addEmpPrompts[0]).then((res) => {
          ...
          inquirer.prompt(addEmpPrompts[1]).then(async (res) => {
            empProps.push(res.last_name);
            optionsQuery('SELECT title FROM role', 'title').then(() => {
              return inquirer.prompt(addEmpPrompts[2]).then(async (res) => {
                ...
                await optionsQuery(
                  ...
                  console.log(options);
                  return inquirer
                    .prompt(addEmpPrompts[3])
                    .then((res) => empProps.push(res.manager));
                });
              });
            });
          });
