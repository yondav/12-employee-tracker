...
const addDepPrompt = {
  name: 'dep_name',
  type: 'input',
  message: chalk.hex('#74E485').bgHex('#000000')('Department Name'),
};
...
const addRolePrompts = [
  {
    name: 'title',
    type: 'input',
    message: chalk.hex('#74E485').bgHex('#000000')('Title'),
  },
  {
    name: 'salary',
    type: 'input',
    message: chalk.hex('#74E485').bgHex('#000000')('Starting Salary -- DECIMAL FORMAT __ DO NOT INCLUD $ --'),
  },
  {
    name: 'department',
    type: 'list',
    message: chalk.hex('#74E485').bgHex('#000000')('Department'),
    choices: depOptions,
  },
];
...
const empEdit = [
  {
    name: 'emp_select',
    type: 'list',
    message: chalk.hex('#74E485').bgHex('#000000')('Select An Employee'),
    choices: empOptions,
  },
  {
    name: 'emp_params',
    type: 'list',
    message: chalk.hex('#74E485').bgHex('#000000')('What Would You Like To Update?'),
            ...
            connection.query('SELECT id FROM employee WHERE last_name = ?', emp[0], (err, res) => {
              ...
              inquirer.prompt(empEdit[1]).then((res) => {
                switch (res.emp_params) {
                  case 'first name':
                    inquirer.prompt(addEmpPrompts[0]).then((res) => {
                      update.update('employee', 'first_name', res.first_name, 'id', empID);
                    });
                    break;
                  case 'last name':
                    inquirer.prompt(addEmpPrompts[1]).then((res) => {
                      update.update('employee', 'last_name', res.last_name, 'id', empID);
                    });
                    break;
                  case 'role':
    ...
    switch (res.add_select) {
      case 'employee':
        ...
        optionsQuery(`SELECT CONCAT(last_name, ', ', first_name) AS name FROM employee;`, 'name', empOptions).then(() => {
          inquirer.prompt(empEdit[0]).then((res) => {
            console.log(res);
            // connection.query(`DELETE FROM employee WHERE first_name = "?"`,  (err, res) => {
